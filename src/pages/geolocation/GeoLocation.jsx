...
function GeoLocation() {
  ...
  const fetchLocations = async () => {
    try {
      ...
      if (response.data) {
        // Assuming the response structure is an object with location names as keys and details as values
        const locationsArray = Object.entries(response.data).map(([name, details]) => ({
          name,
          ...details,
        }));
        ...
        const newCenter = {
          lat: isNaN(minLat) || isNaN(maxLat) ? 0 : (minLat + maxLat) / 2,
          lng: isNaN(minLon) || isNaN(maxLon) ? 0 : (minLon + maxLon) / 2,
        };
